      ...
      >
        ...
        {isUser && !isEditing && (
          <div className="mt-1 opacity-0 group-hover:opacity-100 transition-opacity flex gap-1">
            ...
            <button onClick={() => { setEditContent(message.content); setIsEditing(true); }} className="p-1 text-white/60 hover:text-white" title="편집" aria-label="메시지 편집">
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        ...
        {!isUser && message.content && (
          <div className="mt-1 opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-1">
            ...
            />
            {/* Copy button */}
            <button onClick={handleCopy} className="p-1 text-muted hover:text-foreground" title="복사" aria-label="메시지 복사">
              {copied ? (
            ...
            {isLast && onRegenerate && (
              <button onClick={onRegenerate} className="p-1 text-muted hover:text-foreground" title="재생성" aria-label="응답 재생성">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            ...
            {onBranch && (
              <button onClick={() => onBranch(message.id)} className="p-1 text-muted hover:text-foreground" title="여기서 분기" aria-label="여기서 분기">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                  <line x1="6" y1="3" x2="6" y2="15" />
                  <circle cx="18" cy="6" r="3" />
                  <circle cx="6" cy="18" r="3" />
