...
export default function ChatInput({ onSend, onCommand, disabled, onDrop, planMode, onTogglePlanMode }: ChatInputProps) {
        ...
        <div className="flex gap-2 items-end">
          ...
          {onTogglePlanMode && (
            <button
              ...
              className={`p-3 rounded-xl transition-colors shrink-0 disabled:opacity-40 ${
                planMode
                  ? 'bg-accent/20 text-accent hover:bg-accent/30'
                  : 'text-muted hover:text-foreground hover:bg-card'
              }`}
              title={planMode ? 'Plan 모드 켜짐 — 클릭하여 끄기' : 'Plan 모드 끄기 — 클릭하여 켜기'}
              aria-label="Plan 모드 토글"
            >
          ...
          <div className="flex-1 relative">
            ...
            <textarea
              ...
              onPaste={handlePaste}
              placeholder={isDragOver ? '파일을 여기에 드롭하세요...' : attachedImages.length > 0 ? '이미지에 대해 질문하세요...' : planMode ? '[Plan 모드] 계획할 작업을 입력하세요...' : '메시지를 입력하세요...'}
              disabled={disabled}
              rows={1}
